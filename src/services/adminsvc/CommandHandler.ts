...
export class AdminResetPasswordCommandHandler
  implements ICommandHandler<AdminResetPasswordCommand, boolean>
{
  constructor(private adminRepository: IAdminRepository, private passwordHasher: IPasswordHasher) {}
  async execute(command: AdminResetPasswordCommand): Promise<boolean> {
    const hashedPassword = await this.passwordHasher.hash(command.password);

    return await this.adminRepository.resetPassword(
      command.email,
      hashedPassword,
      command.schoolId
    );
  ...
  implements ICommandHandler<DeleteDisciplineCommand, void>
{
  ...
  async execute(command: UpdateDisciplineCommand): Promise<any> {
    return this.adminRepository.updateDiscipline(command.disciplineId, command.name);
  }
}
